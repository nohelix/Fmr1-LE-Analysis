...
partial.data = c("LP 3", "LP 4", "LP 5", "LP 6", "Purple 1", "Purple 2", "Purple 3", "Purple 4")
```
...
# Thresholds
```{r}
TH.aov = aov(TH ~ Genotype * Duration * `Dur (ms)` + Error(ID), data = TH %>% filter(!(ID %in% partial.data)))
summary(TH.aov)
...
Rxn.aov = aov(LambertW::Gaussianize(Rxn_overall_by_Duration$Rxn) ~ Genotype * Intensity * Duration * `Dur (ms)`, data = Rxn_overall_by_Duration)
...
```{r Save}
save(df, loaded_files, Data_over_TH, Rxn, Rxn_overall_by_Duration, file = "Fmr1_analysis.Rdata")
```
